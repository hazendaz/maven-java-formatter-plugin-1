...
import static net.revelc.code.formatter.AbstractFormatterTest.TEST_OUTPUT_PRIMARY_DIR;
...
class FormatterMojoTest {
    ...
    void setup() throws IOException {
        basedir = TEST_OUTPUT_PRIMARY_DIR.getCanonicalPath();
        sourceFile = new File(TEST_OUTPUT_PRIMARY_DIR, "AnyClass" + FILE_ID.incrementAndGet() + ".java");
     ...
     */
    @Test
    void skip_known_unchanged_java_file() throws Exception {
        formatSourceFile();
     ...
     */
    @Test
    void format_new_dirty_java_file() throws Exception {
        mojo.doFormatFile(sourceFile, rc, cache, basedir, false);
        String cachedHash = (String) cache.get(cacheKey);
        String expectedHash = mojo.sha512hash(FileUtils.fileRead(sourceFile));
        assertEquals(cachedHash, expectedHash);
    ...
    private void formatSourceFile() throws IOException, BadLocationException {
        ...
        formatter.init(Collections.emptyMap(),
                new AbstractFormatterTest.TestConfigurationSource(TEST_OUTPUT_PRIMARY_DIR));
